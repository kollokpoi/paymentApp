const { SubscriptionDTO, PortalDTO, ApplicationDTO, TariffDTO } = require('@payment-app/apiModels');
...
class SubscriptionController {
  ...
  async create(req, res, next) {
    try {
      ...
      const { 
        portal_id, app_id, tariff_id, status, 
        valid_from, valid_until, auto_renew, trial_end_date, notes 
      } = req.body;
      ...
      if (existing) {
        return res.status(400).json({
          success: false,
          message: 'Active subscription already exists for this portal and application'
        });
      }
  ...
  async update(req, res, next) {
    try {
      ...
      await subscription.update({
        status,
        valid_until: valid_until ? new Date(valid_until) : subscription.valid_until,
        auto_renew,
        notes
      });
    ...
    try {
      ...
      if (days) {
        ...
        switch (subscription.tariff.period) {
          case 'day': newValidUntil.setDate(newValidUntil.getDate() + 1); break;
          case 'week': newValidUntil.setDate(newValidUntil.getDate() + 7); break;
          case 'year': newValidUntil.setFullYear(newValidUntil.getFullYear() + 1); break;
          default: newValidUntil.setMonth(newValidUntil.getMonth() + 1);
        }
      }
  ...
  async changeTariff(req, res, next) {
    try {
      ...
      res.json({
        success: true,
        data: subscriptionDTO.toApiResponse(),
        message: immediate ? 'Tariff changed immediately' : 'Tariff will be changed on next renewal'
      });
    } catch (error) {
      next(error);
    }
