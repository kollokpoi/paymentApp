            ...
            <div class="pt-4 border-t">
              ...
              <div v-if="selectedSubscriptionInfo" class="bg-gray-50 p-3 rounded">
                <div class="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div class="font-medium">{{ selectedSubscriptionInfo.portal?.companyName || selectedSubscriptionInfo.portalId }}</div>
                  
                  <div class="text-gray-500">Тариф:</div>
                  <div class="font-medium">{{ selectedSubscriptionInfo.tariff?.name || 'Без тарифа' }}</div>
                  
                  <div class="text-gray-500">Статус:</div>
                  <div>
                    <TagPrime :value="selectedSubscriptionInfo.status" :severity="getSubscriptionStatusSeverity(selectedSubscriptionInfo.status)" />
                  </div>
                </div>
              </div>
...
const isFormValid = computed(() => {
  return !!formData.subscription_id && 
         !!formData.amount && 
         formData.amount > 0 && 
         !!formData.status
})
